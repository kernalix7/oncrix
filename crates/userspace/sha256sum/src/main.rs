...
//! ONCRIX `/bin/sha256sum` — SHA-256 (FIPS 180-4) file digest utility.
//!
//! Forms:
//!   * `sha256sum [FILE...]` — for each FILE print `HEX_HASH  FILENAME\n`
//!     (two spaces). With no operand or with a `-` operand, read stdin and
//!     render the filename as `-`.
//!   * `sha256sum -c FILE` — read FILE as a list of `HEX_HASH  PATH` lines
//!     and verify each PATH. Prints `PATH: OK` or `PATH: FAILED` to stdout
//!     and exits 1 if any check fails.
//!
//! Algorithm: original Rust implementation of SHA-256 as defined by
...
//! mirrors the `sha256sum(1)` behaviour described by GNU coreutils.
...
/// SHA-256 digest length in bytes / hex chars.
...
/// Streaming SHA-256 state. Feed bytes via [`Sha256::update`] and call
/// [`Sha256::finish`] to obtain the 32-byte digest.
struct Sha256 {
...
impl Sha256 {
    /// Construct a new SHA-256 hasher in its initial state.
    fn new() -> Self {
        Self {
            h: H0,
            block: [0u8; 64],
    ...
    ///
    /// Implements FIPS 180-4 § 6.2.2 step-for-step: message schedule
    /// expansion `W[0..64]`, then 64 mixing rounds using the working
    ...
    /// the 32-byte digest. Consumes the hasher state.
...
extern "C" fn sha256sum_main(argc: usize, argv: *const *const u8) -> ! {
    ...
    while i < argc {
        ...
        if first == b'-' && second == b'c' && unsafe { ptr.add(2).read() } == 0 {
            ...
            if nfiles >= MAX_FILES {
                write_all(STDERR, b"sha256sum: too many file arguments\n");
                libc::exit(1);
            }
            files[nfiles] = ptr;
            nfiles += 1;
    ...
    if check_mode {
        ...
        if nfiles == 0 {
            write_all(STDERR, b"sha256sum: -c requires a checksum file\n");
            libc::exit(1);
        }
...
fn hash_operand(path: *const u8) -> bool {
    ...
    if fd < 0 {
        write_all(STDERR, b"sha256sum: cannot open ");
...
/// Stream `fd` through SHA-256 and return the digest. `err_label` is only
...
fn hash_fd(fd: i32, err_label: &[u8]) -> Option<[u8; DIGEST_BYTES]> {
    ...
    loop {
        ...
        if n < 0 {
            write_all(STDERR, b"sha256sum: read error on ");
...
fn check_file(list_path: *const u8) -> bool {
    ...
    loop {
        ...
        if n < 0 {
            write_all(STDERR, b"sha256sum: read error on check list\n");
            libc::close(fd);
            return false;
        }
        let got = n as usize;
...
fn process_check_line(line: &[u8]) -> bool {
    ...
    // Need at least 64 hex chars + two spaces + one path char.
    if line.len() < HEX_CHARS + 3 {
        write_all(STDERR, b"sha256sum: malformed check line\n");
        return false;
    }
    ...
    if path_bytes.len() > MAX_PATH {
        write_all(STDERR, b"sha256sum: path too long in check line\n");
        return false;
    }
...
/// Constant-time equality for two 32-byte digests. Not strictly required by
...
/// Write the digest as 64 lowercase hex characters.
...
fn panic(_info: &PanicInfo) -> ! {
    write_all(STDERR, b"sha256sum: panic\n");
    libc::exit(1)
}
