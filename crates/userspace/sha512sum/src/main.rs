...
//! ONCRIX `/bin/sha512sum` — SHA-512 (FIPS 180-4) file digest utility.
//!
//! Forms:
//!   * `sha512sum [FILE...]` — for each FILE print `HEX_HASH  FILENAME\n`
//!     (two spaces). With no operand or with a `-` operand, read stdin and
//!     render the filename as `-`.
//!   * `sha512sum -c FILE` — read FILE as a list of `HEX_HASH  PATH` lines
//!     and verify each PATH. Prints `PATH: OK` or `PATH: FAILED` to stdout
//!     and exits 1 if any check fails.
//!
//! Algorithm: original Rust implementation of SHA-512 as defined by
...
//! mirrors the `sha512sum(1)` behaviour described by GNU coreutils.
...
/// SHA-512 digest length in bytes / hex chars.
const DIGEST_BYTES: usize = 64;
const HEX_CHARS: usize = 128;
...
/// Initial hash value H(0) — first 64 bits of the fractional parts of the
/// square roots of the first eight primes (FIPS 180-4 § 5.3.5).
const H0: [u64; 8] = [
...
/// Streaming SHA-512 state. Feed bytes via [`Sha512::update`] and call
/// [`Sha512::finish`] to obtain the 64-byte digest.
struct Sha512 {
...
impl Sha512 {
    /// Construct a new SHA-512 hasher in its initial state.
    fn new() -> Self {
        Self {
            h: H0,
    ...
    /// Apply the SHA-512 compression function to one 128-byte block.
    ///
    ...
    /// Append SHA-512 padding (a single `0x80` byte, zero pad to length 112
    /// mod 128, then the 128-bit big-endian message length in bits) and emit
    /// the 64-byte digest. Consumes the hasher state.
...
extern "C" fn sha512sum_main(argc: usize, argv: *const *const u8) -> ! {
    ...
    while i < argc {
        ...
        if first == b'-' && second == b'c' && unsafe { ptr.add(2).read() } == 0 {
            ...
            if nfiles >= MAX_FILES {
                write_all(STDERR, b"sha512sum: too many file arguments\n");
                libc::exit(1);
            }
            files[nfiles] = ptr;
            nfiles += 1;
    ...
    if check_mode {
        ...
        if nfiles == 0 {
            write_all(STDERR, b"sha512sum: -c requires a checksum file\n");
            libc::exit(1);
        }
...
fn hash_operand(path: *const u8) -> bool {
    ...
    if fd < 0 {
        write_all(STDERR, b"sha512sum: cannot open ");
...
/// Stream `fd` through SHA-512 and return the digest. `err_label` is only
...
fn hash_fd(fd: i32, err_label: &[u8]) -> Option<[u8; DIGEST_BYTES]> {
    ...
    loop {
        ...
        if n < 0 {
            write_all(STDERR, b"sha512sum: read error on ");
...
fn check_file(list_path: *const u8) -> bool {
    ...
    loop {
        ...
        if n < 0 {
            write_all(STDERR, b"sha512sum: read error on check list\n");
            libc::close(fd);
            return false;
        }
        let got = n as usize;
...
fn process_check_line(line: &[u8]) -> bool {
    ...
    // Need at least 128 hex chars + two spaces + one path char.
    if line.len() < HEX_CHARS + 3 {
        write_all(STDERR, b"sha512sum: malformed check line\n");
        return false;
    }
    ...
    if path_bytes.len() > MAX_PATH {
        write_all(STDERR, b"sha512sum: path too long in check line\n");
        return false;
    }
...
/// Constant-time equality for two 64-byte digests. Not strictly required by
...
/// Write the digest as 128 lowercase hex characters.
...
fn panic(_info: &PanicInfo) -> ! {
    write_all(STDERR, b"sha512sum: panic\n");
    libc::exit(1)
}
