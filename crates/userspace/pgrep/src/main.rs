...
//! ONCRIX `/bin/pgrep` — procps `pgrep(1)` utility.
//!
//! Looks up PIDs of running processes matching a given name and prints them
//! to stdout, one per line. ONCRIX currently has a fixed early-boot process
//! table, so this implementation recognises a small set of well-known names:
//!
//! * `init` → PID 1
//! * `sh`   → PID 2
//!
//! Anything else (or a missing pattern) exits with status 1, matching procps
//! `pgrep` semantics for "no processes matched".
//!
//! Reference: procps `pgrep(1)`.
...
extern "C" fn pgrep_main(argc: usize, argv: *const *const u8) -> ! {
    // No pattern given — procps "no matches".
    if argc < 2 {
        libc::exit(1)
    }
