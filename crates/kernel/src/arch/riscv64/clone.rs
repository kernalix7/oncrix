...
//! RISC-V 64-bit `fork(2)`/`clone(2)` architecture glue.
//!
...
//! type-check when the kernel is compiled for riscv64. The runtime
//! bodies are placeholders: the riscv64 context switch and the ring 0 →
//! U-mode transition that a real fork requires are not implemented yet.
...
pub struct ForkSnapshot {
    /// Parent's user-mode instruction pointer (riscv64: `sepc`).
    pub user_rip: u64,
    /// Parent's user-mode stack pointer.
    pub user_rsp: u64,
    /// Parent's user-mode flags register (riscv64: `sstatus`).
    pub user_rflags: u64,
...
impl ForkSnapshot {
    ...
    ///
    /// Keeps only the arithmetic/status flags and force-sets the
...
///
/// riscv64 build stub — not yet functional. The signature matches the
/// x86_64 implementation so shared dispatch code type-checks; calling it
/// currently panics because the riscv64 thread-bootstrap path is unwritten.
pub fn arch_clone_thread(_parent: &Thread, _snapshot: &ForkSnapshot) -> Result<Thread> {
    unimplemented!("riscv64: arch_clone_thread not yet implemented")
}

/// Return an untyped pointer to the fork trampoline.
///
/// riscv64 build stub — not yet functional; returns `0` (no trampoline
/// has been assembled for riscv64 yet).
